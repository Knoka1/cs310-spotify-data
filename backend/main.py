...
app.add_middleware(
    ...
    allow_origins=["https://prod.d21eodpbj8ez60.amplifyapp.com"],
...
def deduplicate_albums(albums: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for album in albums:
        name = normalize_album_name(clean_album_name(album.get("name", "")))
...
def calculate_artist_metrics(artist_id: str) -> Dict[str, Any]:
    ...
    if not albums:
        return {
            ...
            "career_momentum": {
                "first_album_year": None,
                "latest_album_year": None,
                "first_release_date": None,
                "latest_release_date": None,
                "total_albums": 0,
                "albums_last_3_years": 0,
                "average_years_between_releases": None,
                "career_span": None,
                "release_frequency": None,
                "recent_share_of_catalog": None,
            },
            "catalog_depth": {
                "total_albums": 0,
                "total_tracks": 0,
                "average_tracks_per_album": None,
                "longest_album": None,
                "shortest_album": None,
                "oldest_album": None,
    ...
    for album in albums:
        ...
        current_album_summary = {
            "name": clean_album_name(album.get("name")),
            "release_date": release_date,
            "total_tracks": track_count,
        }

        if longest_album is None or track_count > longest_album["total_tracks"]:
            longest_album = current_album_summary.copy()

        if shortest_album is None or track_count < shortest_album["total_tracks"]:
            shortest_album = current_album_summary.copy()

        if release_date:
            if oldest_album is None or release_date < oldest_album["release_date"]:
                oldest_album = current_album_summary.copy()

            if most_recent_album is None or release_date > most_recent_album["release_date"]:
                most_recent_album = current_album_summary.copy()
    ...
    if len(release_years) > 1:
        gaps = [
            release_years[i] - release_years[i - 1]
    ...
    recent_share_of_catalog = None
    if total_albums > 0:
        recent_share_of_catalog = round(albums_last_3_years / total_albums, 2)
    ...
    return {
        ...
        "career_momentum": {
            ...
            "recent_share_of_catalog": recent_share_of_catalog,
        },
        "catalog_depth": {
            "total_albums": total_albums,
            ...
            "oldest_album": oldest_album,
            "most_recent_album": most_recent_album,
        },
        "albums": sorted(
            album_summaries,
...
def get_artist_albums(artist_id: str, limit: int = Query(10, ge=1, le=50)):
    """Get an artist's albums."""
    return spotify_request(
        f"artists/{artist_id}/albums",
        {"limit": limit, "include_groups": "album"}
    )
...
def get_artist_eras(artist_id: str):
    ...
    gaps = [
        e["gap_from_previous_days"]
        for e in eras
        if e["gap_from_previous_days"] is not None
    ]
        ...
        "timeline": [
            {
                ...
                "gap_label": (
                    f"{e['gap_from_previous_years']} years since last album"
                    if e["gap_from_previous_years"]
                    else "Debut album"
                ),
                "cover_image": next(
                    (img["url"] for img in parsed[i]["images"] if img.get("height") == 640),
